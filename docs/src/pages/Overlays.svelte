...
<ConfigProvider
  components={{
    Dialog: { animated: motion },
    Drawer: { animated: motion },
    Popover: { animated: motion },
    Tooltip: { animated: motion },
  }}
>
  <StyleProvider {scope} dir={rtl ? 'rtl' : 'ltr'}>
    ...
    <label><input type="checkbox" bind:checked={motion} />启用动效</label>
    ...
    <Popover aria-label="长内容面板" slotProps={{ body: { 'aria-label': '长面板滚动区' } }}>
      {#snippet trigger(props)}<button type="button" {...props}>打开长内容面板</button>{/snippet}
      {#each Array(80).keys() as index (index)}<p>较长的面板内容 {index + 1}</p>{/each}
    </Popover>
