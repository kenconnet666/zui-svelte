...
后续收敛：本文保留源码研究与候选推导。当前 API 取舍及实施顺序统一见 [Core 生产重构统一规划](core-remaining-plan.md)；defineThemeSchemes 不再列为必需新增 API，双方案预输出改为满足首屏目标的候选实现，原 T0–T6 不作为独立路线执行。

最新确认：基础主题不含系统 Token 或视觉值，只保留标准 CSS 能力。lightTheme/darkTheme 从该基础扩展，用户也可直接 extend 基础主题创建任意自有主题；不要求用户填写系统 Token 空模板。具体合同见统一规划第 15 节。
