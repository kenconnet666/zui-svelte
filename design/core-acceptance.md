...
输出后端增量：逻辑有序存储、64 记录分片与 SSR 协议 7 已实现。4 项聚焦 Chrome 回归验证 1,000 规则节点上限、单分片更新、失败重试、反向接管与 metadata 原子校验；Node 相关 10 项通过。A28/A32/A38 的完整三浏览器/规模时间预算仍须对应 CI，不能用此局部证据替代。

主题/API 增量：baseTheme 与亮暗独立扩展、resolved/scope/runtime 命名迁移、统一 schema 校验已实现；基础主题类型负例、无系统变量 SSR、字体双值类别与颜色配对局部通过。A01/A02/A21–A24/A26/A36 的最终包外/浏览器证据等待对应 CI，不据局部结果标记整项完成。

生产重构增量（2026-09-18）：已恢复并修复 A08 普通 marker 文本误判/重复编译协议检查和 A35 表达式行列映射缺陷，编译器/模块 14 项局部回归通过。完整候选 CI 和真实 HMR 等剩余组合尚待验证，原表的“部分证据”不自动升级为完成。
