...
已授权并实施 API 改名与重复入口删除、内部协议标记、共享主题校验。首屏选择由宿主请求配置与 scope 负责，不新增 schemes 工厂；是否进一步提供本地偏好首帧初始化工具是后续独立需求，不作为已提供的 API 宣传。
...
baseTheme 已作为公开名称实现为 defineTheme({})，不引入新主题类型或独立 CSS 语法。CSS 元数据属于 builder，不能复制进每个主题的 Token 定义。基础主题是可用的空 Token 主题，不是尚未完成的值模板。

```ts
// 当前公开 API，调用位置须接入编译器或显式宿主。
