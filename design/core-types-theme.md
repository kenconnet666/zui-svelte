...
2026-09-18 决策更新：继续分类 Token 结构，core 内置亮色/暗色两套并默认亮色；用户通过 extend 扩展并推导类型，主题切换纳入首版。源码依据、具体合同和实施批次见 [主题生产规划](theme-production-plan.md)。本文后续历史候选写法不覆盖已确认决定，新增 API 仍以讨论稿状态标注。

2026-09-18 补充：typed css、主题类别、别名、作用域与首版验收按 [首版生产可用规划](core-production-plan.md) 继续讨论和实现。本文保留前期方案比较。
