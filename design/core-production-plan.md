...
当前 API 重构取舍、实施顺序与事实核对统一见 [Core 生产重构统一规划](core-remaining-plan.md)。本文第 2 节及初始 CI 状态保留为历史基线；A01–A40 与生产门槛继续有效，新增提案不能据此自动视为已获用户确认。
...
8. 第三方组件不以“相对 .svelte 导入”猜测支持能力。当前首版接入统一在组件边界回退完整规则，保证普通 class 转发；以后只有具备明确版本化消费能力的组件才能启用跨组件提升，性能优化不作为正确转发的前提。
...
2026-09-18 用户已取消 token/周额度保留线及定期用量检查，后续不主动轮询额度。按统一规划的质量与完成条件推进，性能预算和产品资源约束不变。
