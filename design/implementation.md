...
补齐 core README 的 builder、宿主方法、资源所有权、同步回调、SSR 时机与支持/升级矩阵。公共 API 快照增加 runtime.binding() 返回类型的成员，防止仅记录 StyleBinding 名字而遗漏其方法变化；没有增加业务 API。相关 core 构建、合同生成、脚本 lint/diff 检查通过：40 个公开导出、45 个合同条目，完整浏览器 gzip 43954 字节、基础主题窄入口 2070 字节、声明 552950 字节。

58eab65529c3104bdd87b22a53947d08c772d906 的 CI 35368083124 已通过所有产品检查，包含主题重入、保留堆和主题切换预算；仍仅在最终归档证据步骤遇到同一文件缺失。该问题已由 48a5786 修复，CI 35368499416 在此次下一批推送检查时仍运行，不等待或轮询。最终目标仍需最新候选完整 CI 与下载产物一致性确认。

## 完整候选及最终文档/编辑器补验

48a5786 与 ff7bf3b 的完整 CI 分别为 35368499416、35368949928，均成功。实际下载两个候选的 workspace-dist；归档 hash 与清单一致，48a5786 的六份原始报告也逐项核对 hash。ff7bf3b 对应 40 个公开导出、45 个合同条目，所有产品门槛与成功产物上传闭合。

逐项审计时补齐规划要求的 65 个默认 Token 用途/配对/亮暗值清单，并通过 presets.test.ts 检查文档与实际主题的一致性，相关 5 项通过。语言服务验收新增 500 Token 诊断与补全，本机独立客户端 VERIFIED，已有正负例反复修正和跨文件语义也通过；大主题暖服务耗时约 1.64 秒，不包含启动时间。所有临时探针已清理。

本批只有文档和验收探针变更，没有扩展业务组件或新增产品 API；最终目标等待该最新提交 CI 验证，推送后仍不等待或轮询。
...
6c4c643 的完整 CI 35370858768 成功，包括自定义基础主题的生产三浏览器与独立包完整链路。继续按规划审计哈希保护，强制 hashText 返回相同值：普通绑定正确拒绝并保持旧快照，但同模块 owned 快速路径跳过内容比对，新增用例在旧实现失败。现同模块也比对 canonical，不静默复用冲突样式；跨模块及引用回收同时断言。

相关 collision/definitions 共 5 项通过；测试及其导入的聚焦 tsc、WebStorm definitions.ts、lint/格式检查通过。只增加缺失保护，不改哈希算法、公开 API 或协议。完整候选仍交 CI，推送后不等待。

## Windows CRLF 值边界修正

fec0f04 的完整 CI 35371406019 成功。值边界审查发现反斜杠续行只消费 CR，随后把 LF 当作未转义换行，错误拒绝合法 CSS 字符串。新增跨 LF/CR/CRLF/FF 的回归在旧实现失败；修复后 13 项相关单元通过。新增 Chrome 原生 style.content 对照确认 CRLF 输入得到相同的 ab 内容，未转义换行仍拒绝。WebStorm、lint/格式检查通过；不改 API、协议或提升白名单。新候选继续交 CI，推送后不等待。
