...
精确十进制正在评估 decimal.js（必要时与更小的 big.js 比较），尚未安装或确定依赖。若引入，职责是 Svelte 数值领域及 Zod/序列化适配，不进入 core 的 CSS 引擎，不替代所有普通 number。不得同时预装多套 decimal 实现；精度/舍入、编辑中间态、类实例快照与 SSR 传输先明确，详见组件规划第 3 节。
