...
需要先做 CSS 作用域原型：组件变量默认在自己的组件边界建立，并按实际使用键输出。同名嵌套组件应有自己的默认边界，不能被外层实例覆盖无意污染；整类配置通过既有 Provider class 显式应用到各实例。跨 Portal 的子部件必须携带/重建所属组件变量作用域，不能依赖原 DOM 继承。ThemeScope 在内部嵌套时也要验证别名求值位置，不能用默认亮色预先折成常量。
