...
状态：待用户审阅后启动。本文只记录执行顺序、交付物与退出门槛；业务 API/类型/slotProps/Zod/Decimal 的唯一合同仍是 [组件主规划](svelte-components.md)，依赖职责见 [依赖取舍](dependencies.md)。不重新维护一份架构草案。
...
当前审阅的重点是上述阶段边界、优先顺序和交付门槛。本计划已写好，但用户尚未下达第一阶段正式开工指令。
